...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
	...
	switch (request.params.name) {
		case "remember_fact": {
			...
			// 1. Keep the JSON backup (optional, but good for debugging)
			data.push(newEntry);
			await fs.writeFile(MEMORY_FILE, JSON.stringify(data, null, 2));

			// 2. Generate Embedding
			const embedding = await embed(fact, { pooling: 'mean', normalize: true });

			// We convert embedding.data (Float32Array) to a normal Array for safety
			await index.insertItem({
				vector: Array.from(embedding.data),
				metadata: { factId: newEntry.id, fact, tags: newEntry.tags }
			});
		...
		case "recall_facts": {
			...
			const results = await index.queryItems(queryEmbedding.data, 3);

			if (results.length === 0) {
				return { content: [{ type: "text", text: "No relevant memories found." }] };
			}

			const formatted = results.map(r => `- ${r.item.metadata.fact} (Tags: ${r.item.metadata.tags}) (Score: ${r.score})`).join("\n");
